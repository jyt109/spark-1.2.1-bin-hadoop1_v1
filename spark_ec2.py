...
def setup_spark_cluster(main, opts):
    ...
    #############################################
    # Set up all the required packages and
    print 'Installing all the required python modules on mains and subordinates'
    ssh(host=main, opts=opts, command="sh spark-ec2/initialize.sh")
    ##################################################
...
#
#
# def scp_command(opts, filename):
#     return ['scp'] + scp_args(opts, filename)
#
#
# def scp(host, opts, filename):
#     try:
#         subprocess.check_call(scp_command(opts, filename) + ['%s@%s' % (opts.user, host)])
#     except subprocess.CalledProcessError as e:
#         print e
